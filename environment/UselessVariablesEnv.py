...
class UselessVariablesEnv(py_environment.PyEnvironment):
    def __init__(self):
        self.duration = 30
        self.size = 10
        ...
        # Specs
        self.initial_observation = np.zeros((2,self.size), dtype=np.float32)
        ...
        self._action_spec = array_spec.BoundedArraySpec(
            shape=(2,self.size), dtype=np.float32, minimum=1, maximum=100, name='action')
        
        self._observation_spec = array_spec.ArraySpec(
            shape = (2,self.size,),dtype=np.float32,name = 'observation')
